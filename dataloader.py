...
class Spectrum_dataset(Dataset):
    ...
    def gen_rays_spectrum(self, gateway_pos, gateway_orientation):
        ...
        Returns
        -------
        r_o : tensor. [n_rays, 3]. The origin of rays　ワールド座標での受信機の位置
        r_d : tensor. [n_rays, 3]. The direction of rays, unit vector　角度
        """
        ...
        R = torch.from_numpy(Rotation.from_quat(gateway_orientation).as_matrix()).float() #3×3
        # Rotation.from_quat(gateway_orientation).as_matrix()は四元数に対応する3D回転行列を表す
