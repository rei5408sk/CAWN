...
class Bartlett():
    ...
    def __init__(self, frequency=5.215e9): 
...
if __name__ == '__main__':
    ...
    filename = "phase.csv" # 1行目に1つ目のpcapに関するアンテナに対応する位相、2行目以降も同様
    phase = []
    ...
    with open(filename, 'r', encoding='utf-8') as f:
        ...
        j = 1
        for row in reader:
            phase = [float(i) for i in row]
            worker = Bartlett()
            spectrum = worker.gen_spectrum(phase)
            spectrum = (spectrum * 255).astype(np.uint8)

            string = f'{j}.png'
            imageio.imsave(string, spectrum)
