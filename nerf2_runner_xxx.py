...
## こちらは使わない方。customが上手く動作していない、
# また別のrunner.pyはそれが上手くできているのでそちらを使う
...
class NeRF2_Runner():
    ...
    # 以下では上手く動作しなかったので無し(1/14)->いつか改良
    # 任意の位置での受信信号のスペクトル予測を行うコード
    # def predict_spectrum_with_custom_tx(self, Tx_loc):
    #     """
    #     Predict the spectrum for a custom transmitter location.

    #     Parameters:
    #     -----------
    #     Tx_loc : list or tensor
    #         The custom transmitter location specified as [x, y, z].

    #     Returns:
    #     --------
    #     pred_spectrum_np : numpy array
    #         The predicted spectrum in a 2D array (elevation x azimuth).
    #     """
    #     self.logger.info(f"Start predicting spectrum with custom Tx_loc={Tx_loc}")
    #     self.nerf2_network.eval()

    #     # 受信スペクトル保存用ディレクトリ作成
    #     save_dir = os.path.join(self.logdir, self.expname, 'custom_pred_spectrum')
    #     os.makedirs(save_dir, exist_ok=True)
    #     pred2 = torch.zeros((0))

    #     # 送信機の位置を設定
    #     Tx_loc_tensor = torch.tensor(Tx_loc, device=self.devices).unsqueeze(0)  # Shape: [1, 3]

    #     with torch.no_grad():
    #         for test_input, test_label in self.test_iter:
    #             # test_input から rays_o, rays_d を取得
    #             test_input, test_label = test_input.to(self.devices), test_label.to(self.devices)
    #             rays_o, rays_d = test_input[:, :3], test_input[:, 3:6]  # rays_o: [batchsize, 3], rays_d: [batchsize, 3]

    #             # 送信機位置をバッチサイズに合わせて複製
    #             Tx_loc_batched = Tx_loc_tensor.repeat(rays_o.shape[0], 1)

    #             # 予測スペクトルを計算
    #             pred_spectrum = self.renderer.render_ss(Tx_loc_batched, rays_o, rays_d)
                
    #             pred2 = pred_spectrum[1*360*90:] #サイズを整える

    #             # スペクトルの形状を確認
    #             self.logger.info(f"Shape of predicted spectrum: {pred_spectrum.shape}")

    #             # スペクトルを2D配列に変換
    #             num_elements = pred_spectrum.numel()
    #             expected_size = 360 * 90
    #             if num_elements == expected_size:
    #                 pred_spectrum_np = pred_spectrum.cpu().numpy().reshape(90, 360)
    #             else:
    #                 self.logger.error(f"Unexpected spectrum size: {num_elements}. Skipping this batch.")
    #                 continue

    #             # 最大値の角度を計算
    #             max_index = np.argmax(pred_spectrum_np)
    #             elevation_idx = max_index // 360
    #             azimuth_idx = max_index % 360
    #             self.logger.info(f"Max value at Azimuth {azimuth_idx}°, Elevation {elevation_idx}°")

    #             # スペクトルを保存
    #             save_path = os.path.join(save_dir, "predicted_spectrum.txt")
    #             np.savetxt(save_path, pred_spectrum_np)
    #             self.logger.info(f"Predicted spectrum saved to {save_path}")

    #             # 1つのバッチのみで処理を終了
    #             break  # Remove this if all batches need to be processed

    #     self.logger.info("Custom spectrum prediction completed.")
    #     return pred_spectrum_np
